...
if test
    ...
    q_target = [0;pi];
    if simul == true
        if ~exist('./transfer_vi_hold/', 'dir')
           mkdir('./transfer_vi_hold/')
        end
        T_simul = 0.0001;
        ...
        for j = 1:length(T_list)
            tic
            T = T_list(j);
            load(sprintf('./inputs_vi_hold/U_%f_endtime_%f.mat', T, T_end), 'U');
            for i = 1:T_end/T
                U_simul((T/T_simul * (i-1)+1) : (T/T_simul*i) ) = U(i) * T_simul/T;
            end
            qf = forward2(q_0, q_m, U_simul, T_simul, T_end);
            loss = norm(qf(2:2:end) - ones(length(qf)/2, 1) * q_target(2))^2;
            save(sprintf('./transfer_vi_hold/q_%f_siumultime_%f_endtime_%f.mat', T, T_simul, T_end), 'qf');
            save(sprintf('./transfer_vi_hold/loss_%f_simultime_%f_endtime_%f.mat', T, T_simul, T_end), 'loss');
            ...
            title('q_1');
            legend('real', 'target');
            saveas(gcf, sprintf('./transfer_vi_hold/q1_%f_endtime_%f.jpg', T, T_end));
            if (fig) 
                figure(3*j-1);
            else
                figure('visible','off');
            ...
            title('q_2');
            legend('real', 'target');
            saveas(gcf, sprintf('./transfer_vi_hold/q2_%f_endtime_%f.jpg', T, T_end))
            if (video)
                v = VideoWriter(sprintf('./transfer_vi_hold/video_%f_endtime_%f.avi', T, T_end));
                open(v);
        ...
        if ~exist('./inputs_vi_hold/', 'dir')
           mkdir('./inputs_vi_hold/')
        end
        for j = 1: length(T_list)
            tic
            ...
            U_simul = U;
            T_simul = T_list(j);
            save(sprintf('./inputs_vi_hold/U_%f_endtime_%f.mat', T_list(j), T_end), 'U');
            qf = forward2(q_0,q_m, U_simul, T_simul, T_end);
            toc
            ...
            title('q_1');
            legend('real', 'target');
            saveas(gcf, sprintf('./inputs_vi_hold/q1_%f_endtime_%f.jpg', T_simul, T_end));
            if (fig) 
                figure(3*j-1);
            else
                figure('visible','off');
            ...
            title('q_2');
            legend('real', 'target');
            saveas(gcf, sprintf('./inputs_vi_hold/q2_%f_endtime_%f.jpg', T_simul, T_end))
            if (video)
                v = VideoWriter(sprintf('./inputs_vi_hold/video_%f_endtime_%f.avi', T_simul, T_end));
                open(v);
                for k = scale : scale :T_end/T_simul
                    figure(3*j)
                    q1 = qf(2*k-1);
...
if test
    ...
    open(v);
    for k = T_end/T_test /50:T_end/T_test /50 :T_end/T_test
        figure(3)
        q1 = qf(2*k-1);
        q2 = qf(2*k);
        x = zeros(2, 2);
...
elseif (simul)
        ...
        save('./transfer_vi_hold/loss_list_vi', 'loss_list');
        save('./transfer_vi_hold/T_list_vi', 'T_list');
end
...
function f = ObjFunc2(U, q_0, q_m, q_target, T, T_end) % objective function without gradient
    ...
    f = ((q(2:2:end) - q_target(2) * ones(length(q)/2, 1))') *  (q(2:2:end) - q_target(2) * ones(length(q)/2, 1));
end
