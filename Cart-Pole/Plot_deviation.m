...
for j = 1:length(T_list)
    load(sprintf('./test_vi/q_%f_endtime_%f.mat', T_list(j), T_end), sprintf('q_%d', j));
end
for j = 1:length(T_list)
...
semilogx(T_list(1:end), dev_list(1:end, 1), 'linewidth', 3);
hold on;
for j = 1:length(T_list)
...
semilogx(T_list(1:end), dev_list(:, 3), 'linewidth', 3);
for j = 1:length(T_list)
...
semilogx(T_list(1:end), dev_list(:, 2), 'linewidth', 3);

legend('variational integrator', 'Euler', 'PMI');
