...
if test
    ...
    q_target = [0; pi];
    if simul == true
        if ~exist('./transfer_pmi_hold/', 'dir')
           mkdir('./transfer_pmi_hold/')
        end
        T_simul = 0.0001;
        ...
        for j = 1:length(T_list)
            tic
            T = T_list(j);
            load(sprintf('./inputs_pmi_hold/U_%f_endtime_%f.mat', T, T_end), 'U');
            for i = 1:T_end/T
            ...
            save(sprintf('./transfer_pmi_hold/q_%f_siumultime_%f_endtime_%f.mat', T, T_simul, T_end), 'qf');
            save(sprintf('./transfer_pmi_hold/loss_%f_simultime_%f_endtime_%f.mat', T, T_simul, T_end), 'loss');
            ...
            title('q_1');
            legend('real', 'target');
            saveas(gcf, sprintf('./transfer_pmi_hold/q1_%f_endtime_%f.jpg', T, T_end));
            if (fig) 
                figure(3*j-1);
            else
                figure('visible','off');
            ...
            title('q_2');
            legend('real', 'target');
            saveas(gcf, sprintf('./transfer_pmi_hold/q2_%f_endtime_%f.jpg', T, T_end))
            if (video)
                v = VideoWriter(sprintf('./inputs_pmi_hold/video_%f_endtime_%f.avi', T_simul, T_end));
                open(v);
        ...
        if ~exist('./inputs_pmi_hold/', 'dir')
           mkdir('./inputs_pmi_hold/')
        end
        for j = 1: length(T_list)
            tic
            ...
            U_simul = U;
            T_simul = T_list(j);
            save(sprintf('./inputs_pmi_hold/U_%f_endtime_%f.mat', T_list(j), T_end), 'U');
            [qf, ~, ~, ~] = forward(q_0,v_0, U_simul, m, L, g, J, T_simul, T_end);
            toc
            ...
            title('q_1');
            legend('real', 'target');
            saveas(gcf, sprintf('./inputs_pmi_hold/q1_%f_endtime_%f.jpg', T_simul, T_end));
            if (fig) 
                figure(3*j-1);
            else
                figure('visible','off');
            ...
            title('q_2');
            legend('real', 'target');
            saveas(gcf, sprintf('./inputs_pmi_hold/q2_%f_endtime_%f.jpg', T_simul, T_end))
            if (video)
                v = VideoWriter(sprintf('./inputs_pmi_hold/video_%f_endtime_%f.avi', T_simul, T_end));
                open(v);
                for k = scale : scale :T_end/T_simul
...
function f = ObjFunc(U, q_0, q_m, q_target, m, L, g, J, T, T_end) % objective function without gradient
    ...
    f = 20 * norm(q(2:2:end) - q_target(2)*ones(length(q)/2))^2 + 1/500*(U')*U;
end
